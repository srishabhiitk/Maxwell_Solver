...
int main(){
  ...
  for (i=0; i<num_timesteps; i++){
    if (i==0){
      ...
      // E->z(N/2,N/2,N/2)=1.0;
      E->z(16, 16 ,N/2)=exp(-pow(-init+i,2)/sigma);
    ...
    // std::cout<<Hadv->x(rd)<<std::endl;

    upBound=N-1,N-2,N-1;
    rd=blitz::RectDomain<3>(loBound,upBound);
    Hadv->y(rd)=Da*H->y(rd)-Db*Ecurl->y(rd);
    // std::cout<<"Hy:"<<std::endl;
    // std::cout<<Hadv->y(rd)<<std::endl;

    upBound=N-1,N-1,N-2;
    rd=blitz::RectDomain<3>(loBound,upBound);
    Hadv->z(rd)=Da*H->z(rd)-Db*Ecurl->z(rd);
    // std::cout<<"Hz:"<<std::endl;
    // std::cout<<Hadv->z(rd)<<std::endl;
    ...
    E->z(16,16,N/2)=exp(-pow(-init+i,2)/sigma);
    Eadv->z(16,16,N/2)=exp(-pow(-init+i,2)/sigma);
    ...
    std::cout<<i<<", max="<<blitz::max(Etotal->z)<<", Ez="<<E->z(N/2,N/2,N/2)<<std::endl;
